...
Examples:
  $0 i386                    # Test i386 generic build
...
test_qemu() {
    ...
    local target="${2:-generic}"
    local kernel_path
    kernel_path=$(get_kernel_path "$arch" "$target")
    
    print_info "Testing SAGE-OS for $arch architecture (target: $target)"
    ...
    print_info "Starting QEMU for $arch..."
