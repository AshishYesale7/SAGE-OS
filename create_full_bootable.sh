...
# Create full bootable disk image with SAGE OS kernel
set -e

echo "🔧 Creating full SAGE OS bootable disk image with keyboard input..."

# Create a larger disk image (10MB)
dd if=/dev/zero of=sage_os_full.img bs=1024 count=10240

# Create an advanced bootloader that loads our compiled kernel
cat > advanced_bootloader.S << 'EOF'
.code16
.section .text
.global _start
...
echo "📝 Compiling advanced bootloader..."
gcc -m32 -nostdlib -nostartfiles -Wl,--oformat=binary -Wl,-Ttext=0x7C00 advanced_bootloader.S -o advanced_bootloader.bin

echo "💾 Creating full bootable image..."
# Copy bootloader to first sector
dd if=advanced_bootloader.bin of=sage_os_full.img bs=512 count=1 conv=notrunc

echo "✅ Full bootable image created: sage_os_full.img"
echo "🚀 Test with: qemu-system-i386 -fda sage_os_full.img"
echo "🎮 Features: 32-bit protected mode, VGA graphics, keyboard input, interactive shell"